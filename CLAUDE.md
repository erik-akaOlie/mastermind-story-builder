...
**One-liner:** A visual canvas for constructing and navigating story worlds as networks of connected narrative entities — trace relationships, uncover gaps, and develop ideas. (Vocabulary: the entity is a NODE; card and bead are its display states.)
...
**Working model — authority and authorization:**

- **Erik owns** product direction, UX, priorities, terminology, architecture and product
  decisions, and approval for commits, pushes, deployments, promotions, service
  configuration, purchases, domains, and any Production change.
- **CC (Claude) inspects** the repository, **proposes** engineering and architecture,
  **implements** approved work, **tests**, and **documents** it — within approved scope.
- **Standing authorization rule:** questions, reviews, plan agreement, and advisory
  discussion are NOT implementation or external-action approval. Do not commit, push,
  deploy, promote, or configure external services without Erik's explicit approval.
...
**Presenting options & recommendations.** For substantial advisory, inspection, or decision-making turns, default to: Facts / Why it matters / Options with tradeoffs / Recommendation and rationale / Confidence. Use another structure when it communicates the issue more clearly, and do not manufacture options or sections merely to satisfy the format. Skip it for execution, simple factual answers, quick clarifications, and status updates. (Refined 2026-08-04 from "any advisory turn" — the structure's value is separating evidence, consequences, options, recommendation, and uncertainty, not the ritual.)

What each part means when the structure is used:
...
If only one path is viable, say so rather than invent fake options. Don't surface decisions as a bare multiple-choice prompt — the reasoning must be visible. This complements the 7-part structure above: 7-part for deep architecture/systems-design dives, 5-part for everyday decisions.
...
Local actions can mutate real user data. Daily campaign work remains in Production.

- **`master` = integration branch → Preview environment.** Master builds carry real app
  config via branch-pinned env vars (Preview/`master` scope) and are reachable at the
  stable alias `mastermind-story-builder-git-master-erik-akaolies-projects.vercel.app`,
  behind Vercel Authentication. Unpinned branches receive no Production-data-bearing
  variables. In the controlled A8 test (2026-08-13), the current build failed closed:
  startup exception, no sign-in surface, zero Production-host requests, and the Production
  hostname absent from every JavaScript asset loaded and inspected during that test.
- **Env-var table (Vercel):** `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`,
  `VITE_POSTHOG_KEY` each exist twice — Production, and Preview pinned to `master`.
  `VITE_POSTHOG_HOST` remains All Environments (inert; deferred cleanup).
- **GitHub protections (bypass lists empty):** branch ruleset `protect-release-branches`
  (`master`, `production-gate`: no deletion, no force push); tag ruleset
  `protect-release-tags` (`prod-*`, `verification-release-tag-*`: no update/deletion/
  force push).
...
- [x] **Edge-hover dual-expand + stabilized session** — Hovering a connection line expands **both** endpoint nodes into readable cards (Part B of the edge-hover work; Part A shipped the highlight-only pass in `d360640`). Tied to [ADR-0010](./docs/decisions/0010-zoom-progressive-disclosure.md). **Since 2026-07-31 this is altitude-agnostic (Erik's equal-emphasis rule):** Card View edge-highlighted endpoints grow to the peek size too — or by `EDGE_EMPHASIS_GROW` (10%) when already rendering larger; **highlighting never shrinks a card** (`emphasisZoom`, altitude.js). The expansion-eligibility rule is centralized in `isExpandedForm` (altitude.js) — the ONE source consumed by both CampaignNode and App's `currentlyExpandedIds` mirror (Card View expands for edge-highlight ONLY; hover/selection/search expansion stays Bead-View-only). The bead-center anchor generalized to a per-form anchor (bead center vs card center — Card View cards grow in place), so clamp/repulsion/publish reduce to the old formulas in Bead View by construction; bead-style `pointer-events:none` peeks are NOT carried to Card View (cards stay interactive); sessions survive threshold crossings mid-hover. Deferred polish (BACKLOG): pair-aware placement when both endpoints clamp into the same viewport corner. Three layers:
