...
- **Custom node types — localStorage to Supabase.** The schema supports per-campaign types; the UI still writes custom types to localStorage. This is currently fine for Erik but breaks the moment another user creates a custom type. Tracked in CLAUDE.md "Known Divergences."
