...
| **Inspector**       | The surface for editing one node's contents (title, type, image, summary, notes, connections). Opens in two modes the user moves between: an *undocked* draggable floating modal that morphs from the card, or a *docked* bottom-right overlay panel. Single-clicking another card while it's open *repoints* it to that card. Renamed from "edit modal" in [ADR-0015](../decisions/0015-float-or-dock-inspector.md). |
